...
export class EventController {
  ...
  async createEvent(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      const { event_name, event_start_dt, event_end_dt, year, event_description, event_type, workshop_theme, workshop_program_start_dt, workshop_exhibition_dt, workshop_registration_url, rsvp_enabled, location, photo_gallery_link, is_priority, rsvp_link, is_active_durga_puja_event, venue_name, venue_street, venue_city, venue_region, venue_postal, ticket_url, ticket_price, ticket_currency, event_status, performers, performer_type, artist_ids, meta_title, meta_description, image_alt, faqs, menu } = req.body;
