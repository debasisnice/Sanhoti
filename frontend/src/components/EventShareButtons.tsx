import { shareEventOnFacebook, shareEventOnInstagram, shareEventOnWhatsApp } from '../utils/socialShare';

interface EventShareButtonsProps {
  eventId: string;
  eventName: string;
  className?: string;
  /** When false, only the button row is shown (e.g. home page cards). */
  showCaption?: boolean;
}

/**
 * Facebook / WhatsApp / Instagram share actions using the server-rendered share URL (correct preview image).
 */
export default function EventShareButtons({
  ...
  return (
    <div className={`flex flex-col gap-2 ${className}`}>
      {showCaption ? <p className="text-sm font-medium text-gray-700">Share this event</p> : null}
      <div className="flex gap-3 justify-center flex-wrap">
        <button
          type="button"
          onClick={() => shareEventOnFacebook(eventId)}
        ...
        <button
          type="button"
          onClick={() => shareEventOnWhatsApp(eventId, eventName)}
        ...
        <button
          type="button"
          onClick={() => void shareEventOnInstagram(eventId, eventName)}
