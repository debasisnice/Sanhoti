...
echo "   npm run build"
echo "   cd ../backend"
echo "   npm run build   # required: PM2 runs compiled dist/, not TypeScript src"
echo "   cd .."
...
echo "   cd /var/www/sanhoti && git pull origin main && cd frontend && (git diff HEAD@{1} HEAD --name-only 2>/dev/null | grep -q 'package.json\|package-lock.json' && npm ci --prefer-offline || echo 'Skipping npm ci') && npm run build && cd ../backend && npm run build && cd .. && sudo systemctl reload nginx && pm2 restart sanhoti-backend"
echo ""
echo "⚡ Speed optimizations:"
echo "   - Uses --prefer-offline to use npm cache"
echo "   - Skips npm ci if dependencies haven't changed"
echo "   - Builds frontend + backend (backend dist/ must match src for PM2)"
