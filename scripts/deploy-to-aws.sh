...
echo "   cd /var/www/sanhoti && git pull origin main && cd frontend && npm ci && npm run build && cd ../backend && npm run build && cd .. && sudo systemctl reload nginx && pm2 restart sanhoti-backend"
echo ""
echo "⚡ For FASTER deployment (skips npm ci if dependencies unchanged):"
echo "   cd /var/www/sanhoti && git pull origin main && cd frontend && (git diff HEAD@{1} HEAD --name-only 2>/dev/null | grep -q 'package.json\|package-lock.json' && npm ci --prefer-offline || echo 'Skipping npm ci - no dependency changes') && npm run build && cd ../backend && npm run build && cd .. && sudo systemctl reload nginx && pm2 restart sanhoti-backend"
echo ""
