...
# Working Fix for PDF Worker MIME Type
# This matches the actual format in mime.types

set -e

echo "🔧 Working Fix for PDF Worker MIME Type..."
echo ""
...
if grep -q "application/javascript.*mjs" /etc/nginx/mime.types; then
    ...
    echo "   .mjs NOT found. Adding it..."
    
    # Find the JavaScript line (format: application/javascript                js;)
    ...
    if [ -z "$JS_LINE" ]; then
        ...
        grep -n "javascript" /etc/nginx/mime.types
        exit 1
    fi
    ...
    # Add mjs line after JavaScript line (match the spacing format)
...
if grep -q "location /pdfjs" "$NGINX_CONFIG"; then
    ...
    if [ -n "$START_LINE" ]; then
        # Find the closing brace (should be 6 lines after start)
